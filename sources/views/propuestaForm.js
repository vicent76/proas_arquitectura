...
export default class PropuestasForm extends JetView {
                                ...
                                elements: [
                                    ...
                                    },
                                    {
                                        cols: [
                                            ...
                                            {
                                                view: "combo", id: "cmbProfesionesProveedor", name: "tipoProfresionalId", required: true, options: {},
    ...
    load(propuestaId) {
        ...
        if(desdeCoste) { 
            if (savedTab) {
                $$("tabViewPropuesta").setValue(savedTab);  // Activa la pestaña recuperada
            }
        } else if(desdeVenta) {
            if (savedTab) {
                $$("tabViewPropuesta").setValue(savedTab);  // Activa la pestaña recuperada
        ...
        propuestasService.getPropuesta(propuestaId)
            .then((ex) => {
                ...
                this.loadTiposProyecto(propuesta.tipoProyectoId);  
                delete propuesta.empresa;
                delete propuesta.cliente;
                delete propuesta.estado;
                delete propuesta.agente;
                delete propuesta.comercial;
                delete propuesta.jefeGrupo;
                delete propuesta.jefeObras;
                delete propuesta.oficinatecnica;
                delete propuesta.asesorTecnico;
                propuesta.fecha = new Date(propuesta.fecha);
                formaPagoId = propuesta.formaPagoId;
                $$("frmPropuestas").setValues(propuesta);
                //this.loadTiposProfesional(propuesta.tipoProfesionalId);
                //this.loadAgentes(propuesta.agenteId);
                this.loadEmpresas(propuesta.empresaId);
                this.loadClientesAgente(propuesta.clienteId, propuesta.agenteId);
                this.loadEstados(propuesta.estadoPropuestaId);
                this.loadTiposVia(propuesta.tipoViaId);
                this.loadAgentes(propuesta.agenteId)
                this.buscaColaboradoresActivos("", "comercialId", "cmbComerciales", propuesta.comercialId);
                this.buscaColaboradoresActivos("", "jefeGrupoId", "cmbJefeGrupo", propuesta.jefeGrupoId)
                //this.buscaColaboradoresActivos("", "jefeObrasId", "cmbJefeObras", propuesta.jefeObrasId);
                //this.buscaColaboradoresActivos("", "oficinaTecnicaId", "cmbOficinaTecnica", propuesta.oficinaTecnicaId);
                this.buscaColaboradoresActivos("", "asesorTecnicoId", "cmbAsesorTecnico", propuesta.asesorTecnicoId);
                $$("cmbTiposProyecto").unblockEvent();

                ofertasCosteGrid.loadGrid(propuesta.propuestaId, null, propuesta.importeObra, selectOfertaCosteId);
                ofertasVentaGrid.loadGrid(propuesta.propuestaId, null, propuesta.importeObra, selectOfertaVentaId);
                ofertasSubcontrataGrid.loadGrid(propuesta.propuestaId, null, propuesta.importeObra, selectOfertaSubcontrataId)
    ...
    cambioTipoProyecto(tipoProyectoId) {
        ...
        .then( row => {
            if(row) {
                propuestasService.getSiguienteReferencia(row.abrev)
                .then( row => {
...
loadTiposProyecto(tipoProyectoId) {
    ...
    loadNumero() {
        propuestasService.getNumServicio()
        .then(row => {
