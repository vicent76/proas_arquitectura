...
var sistemaPagoTextoCache = null;
var conceptosExcluidosTextoCache = null;
var sinSalir = null
...
export default class OfertasVentaForm extends JetView {
                                ...
                                elements: [
                                    ...
                                    },
                                    {
                                        cols: [
                                            ...
                                            {
                                                view: "label", label: "visualizar en ERP", width: 120, on: {
                                            ...
                                            {
                                                view: "checkbox", id: "visualizaEnErp", name: "visualizaEnErp", width: 50,
                                                on: {
                                                    onItemClick: function () {
                                                        this.$scope.actulizaVisualizaEnErp(this.getValue());
                                                    }
                                                }
                                            },
                                            {
    ...
    load(ofertaId, expedienteId) {
        ...
        sistemaPagoTextoCache = null;
        conceptosExcluidosTextoCache = null;
        if (ofertaId == 0 && expedienteId > 0) {
            expedientesService.getExpediente(expedienteId)
                .then((expediente) => {
                    ...
                    lineasOfertaVenta.loadGrid(null, false, []);
        ...
        ofertasService.getOferta(ofertaId)
            .then((oferta) => {
                ...
                sistemaPagoTextoCache = oferta.sistemaPago;
                conceptosExcluidosTextoCache = oferta.conceptosExcluidos;
                if (sinSalir) tituloTextoCache = null;
                if (sinSalir) sistemaPagoTextoCache = null;
                if (sinSalir) conceptosExcluidosTextoCache = null;

                $$("frmOfertas").setValues(oferta);
                this.loadEmpresas(oferta.empresaId);
                lineasOfertaVenta.loadGrid(oferta.ofertaId, false, []);
    ...
    accept(opcion) {
        ...
        if (ofertaId == 0) {
            ...
            ofertasService.postOfertaVenta(data)
                .then((result) => {
                     this.show('/top/ofertasVentaForm?ofertaId=' + result.ofertaId + '&expedienteId=' + expedienteId + '&importeObra=' + importeObra + '&SinSalir=true&NEW=true');
                })
                .catch((err) => {
                    var error = err.response;
                    var index = error.indexOf("Cannot delete or update a parent row: a foreign key constraint fails");
            ...
            ofertasService.putOfertaVenta(data)
                .then(() => {
                    ...
                    if (opcion) {
                        ...
                        this.show('/top/ofertasVentaForm?ofertaId=' + data.ofertaId + '&expedienteId=' + expedienteId + '&importeObra=' + importeObra + '&SinSalir=true');
                    }
                })
                .catch((err) => {
                    var error = err.response;
    ...
    cambioTextosPredeterminados(id) {
        ...
        if (sistemaPagoTextoCache === null || sistemaPagoTextoCache === '') {
            textosPredeterminadosService.getTexto(id)
                .then((row) => {
    ...
    cambioTextosPredeterminadosExcluidos(id) {
        ...
        if (conceptosExcluidosTextoCache === null || conceptosExcluidosTextoCache === '') {
            textosPredeterminadosService.getTexto(id)
                .then((row) => {
    ...
    loadTituloCoste(presupuestoCosteId) {
        ofertasService.getOferta(presupuestoCosteId)
            .then((row) => {
                if (!row) {
                    messageApi.errorMessage("Fallo al cargar los datos del presupuesto de coste.");
                    return;
                }
                this.loadTitulos(row.empresaId, row.tituloId);
        ...
        ofertasService.getLineasOferta(presupuestoCosteId)
            .then((rows) => {
                if (rows.length == 0) {
                    ...
                    lineasOfertaVenta.loadGrid(ofertaId, false, rows)
                    return;
                }
                ...
                lineasOfertaVenta.loadGrid(ofertaId, true, rows);
    ...
    cargaPorcenRef() {
        ...
        let a = $$('referencia').getValue();
        let abrev = r.split("_")[0]
        ...
        let ref = abrev + "-" + com + "/" + anyo + matchFinal;
    ...
    aceptarGenerarContrato(app) {
        ...
        var ex = $$('cmbExpedientes').getValue();
        if (c == "") c = 0;
        const translate = app.getService("locale")._;
        var m = "Se generará un contrato a partir de esta oferta.\n ¿Seguro que quiere continuar?."
        ...
        webix.confirm({
            ...
            callback: (action) => {
                if (action === true) {
                    generarContratoWindow.loadWindow(ofertaId, c, ex);
                }
            }
        });
    }

    actulizaVisualizaEnErp(valor) {
        let data = {
            ofertaId: ofertaId,
            visualizaEnErp: valor
        }
        ofertasService.putOferta(data)
            .then(() => {
                let c = $$('contratoId').getValue();
                if(c && c > 0){
                    let data2 = {
                        contratoId: c,
                        visualizaEnErp: valor
                    }
                    ofertasService.putContratoAsociado(data2)
                    .then(() => {
                        messageApi.normalMessage('Se ha actualizado la opción de visualizar en ERP correctamente.');
                    })
                    .catch((err) => {
                        messageApi.errorMessageAjax(err);
                    });
