...
import { PropuestasAceptadasGrid } from '../subviews/propuestasAceptadasGrid'
import { contactosExpediente } from "../subviews/contactosExpediente";
...
export default class ExpedientesForm extends JetView {
    config() {
        ...
        const _propuestasAceptadas = PropuestasAceptadasGrid.getGrid(this.app);
        const _contactosExpediente = contactosExpediente.getGrid(this.app);
        const _view = {
            view: "tabview",
            ...
            tabbar: {
                on: {
                    onAfterTabClick: (id, ev) => {
                        ...
                        webix.storage.local.put("tabViewExpedienteActive", id);
                    },
                }
            },
            cells: [
                                ...
                                elements: [
                                    ...
                                    },
                                    {
                                        cols: [
                                            ...
                                            {
                                                view: "combo", id: "cmbHonorariosVariables", name: "honorarioVariableId", required: true,
                                                label: "Honorarios variables (%)", labelPosition: "top", gravity: 1, options: {}
                                            }
                                        ]
                                    },
                                    {
                                    ...
                                    _contactosExpediente,
                                    {
                                        view: "textarea", id: "datosAdicionales", name: "datosAdicionales",
                                        label: "Datos adicionales", labelPosition: "top", height: 140
                ...
                {
                    header: "Propuestas aceptadas",
                    width: 200,
                    body: {
                        view: "layout",
                        id: "propuestasAceptadas",
                        multiview: true,
                        rows: [
                            _propuestasAceptadas
                        ]
                    }
    ...
    urlChange(view, url) {
        ...
        selectPropuestaAceptadaId = null;
        if (url[0].params.NEW) {
        ...
        if (url[0].params.propuestaAceptadaId) {
            selectPropuestaAceptadaId = url[0].params.propuestaAceptadaId;
        }

        const tabs = $$("tabViewExpediente");
        const activeTab = webix.storage.local.get("tabViewExpedienteActive");
        if (activeTab) {
    ...
    load(expedienteId) {
        if (expedienteId == 0) {
            ...
            this.loadHonorariosVariables(1);
            //
            ...
            contactosExpediente.loadGrid(null);
        ...
        expedientesService.getExpediente(expedienteId)
            .then((ex) => {
                ...
                this.loadAgentes(expediente.agenteId);
                this.loadHonorariosVariables(expediente.honorarioVariableId);
                ...
                PropuestasAceptadasGrid.loadGrid(expediente.expedienteId, selectPropuestaAceptadaId);
    ...
    loadHonorariosVariables(honorarioVariableId) {
        if (honorarioVariableId == 0) honorarioVariableId = 1
        var rows = [
            {
                honorarioVariableId: 1,
                nombre: 0
            },
            {
                honorarioVariableId: 2,
                nombre: 2
            }
        ]
        var variables = generalApi.prepareDataForCombo('honorarioVariableId', 'nombre', rows);
        var list = $$("cmbHonorariosVariables").getPopup().getList();
        list.clearAll();
        list.parse(variables);
        $$("cmbHonorariosVariables").setValue(honorarioVariableId);
        $$("cmbHonorariosVariables").refresh();
    }
}