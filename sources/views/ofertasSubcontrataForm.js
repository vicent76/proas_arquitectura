...
export default class OfertasSubcontrataForm extends JetView {
    config() {
        ...
        const _lineasOferta = lineasOferta.getGrid(this.app, true);
    ...
    urlChange(view, url) {
        ...
        if (url[0].params.ofertaCosteId) {
            ofertaCosteId = parseFloat(url[0].params.ofertaCosteId);
        }
            ...
            .then((parametros) => {
                ...
                if (ofertaId == 0 && expedienteId > 0) {
                    expedientesService.getExpediente(expedienteId)
                        .then((expediente) => {
                            ...
                            lineasOferta.loadGrid(null, null, importeObra, null, ofertaCosteId);
                            $$('valorado').setValue(1);
                ...
                ofertasService.getOferta(ofertaId)
                    .then((oferta) => {
                        ...
                        lineasOferta.loadGrid(oferta.ofertaId, _imprimirWindow, importeObra, null, ofertaCosteId);
            ...
            ofertasService.putOferta(data, data.ofertaId)
                .then(() => {
                    if (opcion) {
                        ...
                        _app.show('/top/ofertasSubcontrataForm?ofertaId=' + data.ofertaId + + '&expedienteId=' + expedienteId + '&importeObra=' + importeObra + '&adicional=' + adicional + '&SinSalir=true');
                    }
                })
                .catch((err) => {
                    var error = err.response;
