...
export const lineasOfertaVenta = {
    ...
    getGrid: (app) => {
        ...
        var datatablelineasOfertaVenta = {
            ...
            columns: [
                ...
                {
                    id: "importe", header: [translate("Coste/Ud."), { content: "textFilter" }], sort: "string", width: 80,
                    format: function (value) {
                        // Forzamos el signo negativo
                        if (!value) value = 0;
                        let formatted = webix.Number.format(Math.abs(value), {
                            decimalSize: 4,
                            groupDelimiter: ".",
                            decimalDelimiter: ","
                        });
                    ...
                    css: { "text-align": "right" }, adjust: "all"
                },
                { id: "cantidad", header: [translate("Cant."), { content: "textFilter" }], sort: "string", width: 50, css: { "text-align": "right" } },
                { id: "precio", header: [translate("Precio"), { content: "textFilter" }], sort: "string", width: 80, css: { "text-align": "right" }, hidden: true },
                {
                    id: "coste", header: [translate("Coste linea"), { content: "textFilter" }], sort: "string", minWidth: 80, css: { "text-align": "right" }, adjust: "all",
                    format: function (value) {
                        // Forzamos el signo negativo
                        if (!value) value = 0;
                        let formatted = webix.Number.format(Math.abs(value), {
                ...
                {
                    id: "ventaNetaLinea", header: [translate("venta neta"), { content: "textFilter" }], sort: "string", width: 80,
                    format: function (value) {
                        // Forzamos el signo negativo
                        if (!value) value = 0;
                        let formatted = webix.Number.format(Math.abs(value), {
                    ...
                    hidden: true, css: { "text-align": "right" }
                },

                {
                    id: "importeAgenteLinea", header: [translate("Imp. Agente."), { content: "textFilter" }], sort: "string", width: 110,
                    format: function (value) {
                        // Forzamos el signo negativo
                        if (!value) value = 0;
                        let formatted = webix.Number.format(Math.abs(value), {
                ...
                "OnAfterLoad": function () {
                    setTimeout(() => {
                        ...
                        datatable.eachRow(function (rowId) {
                            ...
                            if (row.porcentajeBeneficioLinea || !cargaCoste) {
    ...
    loadGrid: (ofertaid, cargacoste, lineas) => {
        ofertaId = ofertaid;
        var total = 0;
        cargaCoste = cargacoste;
        var total = 0;
