...
import { LineasSubcontrataWindow } from "../subviews/lineasSubcontrataWindow";
...
var isNewRow = false;
var esSubcontrata = false



var ofertaId;
var ofertaCosteId = null;
var numLineas;
var imprimirWindow;
var importeObra = 0;
var contratoId = null;
...
export const lineasOferta = {
    ...
    getGrid: (app, essubcontrata) => {
        esSubcontrata = essubcontrata
        const translate = app.getService("locale")._;
        try {
            LineasOfertaWindow.getWindow(app);
            LineasSubcontrataWindow.getWindow(app)
        } catch (e) {
            console.log(e);
        }
                    ...
                    click: () => {
                        ...
                        try {
                            ...
                            if (!essubcontrata) {
                                ...
                                let ofertaSubcontrataId = ofertaId;
                                LineasSubcontrataWindow.loadWindow(ofertaCosteId, ofertaSubcontrataId, expedienteId);
                            }
        ...
        var datatablelineasOferta = {
            ...
            footer: false,
            ready: function () {
                $$('lineasOfertaGrid').attachEvent("onItemDblClick", function (id, e, node) {
            ...
            columns: [
                ...
                { id: "cantidad", header: [translate("Cant."), { content: "textFilter" }], sort: "string", width: 50, css: { "text-align": "right" } },
                {
                    ...
                    sort: "float", // usar float en vez de string
                    adjust: "all",
                    width: 80,
                ...
                {
                    id: "dto", header: [translate("Descuento"), { content: "textFilter" }], sort: "string", width: 100,
                    format: function (value) {
                        // Forzamos el signo negativo
                        if (!value) value = 0;
                        let formatted = webix.Number.format(Math.abs(value), {
    ...
    loadGrid: (ofertaid, _imprimirWindow, importeobra, contratoid, ofertacosteId) => {
        ...
        if (ofertaId) {
            ofertasService.getLineasOferta(ofertaId)
                .then(rows => {
                    var total = 0;
                    if (rows.length > 0) {
                        expedienteId = rows[0].expedienteId;
