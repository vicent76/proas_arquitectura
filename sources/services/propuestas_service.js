...
export const propuestasService = {
    ...
    getPropuestasSubcontrata: (subcontataId, esGanadora, propuestaId) => {
        return new webix.promise((success, fail) => {
            devConfig.getConfig()
                .then(conf => {
                    var url = conf.urlApi + `/api/propuestas/subcontrata/${subcontataId}/${esGanadora}/${propuestaId}`
                    return webix.ajax()
                        .timeout(10000)
                        .headers({
                            "Content-Type": "application/json"
        ...
        getPropuestasExpediente: (expedienteId, ganadora) => {
        return new webix.promise((success, fail) => {
            devConfig.getConfig()
                .then(conf => {
                    var url = conf.urlApi + `/api/propuestas/expediente/${expedienteId}/${ganadora}`
                    return webix.ajax()
                        .timeout(10000)
                        .headers({
                            "Content-Type": "application/json"
    ...
    putPropuesta: (propuesta) => {
        ...
        putPropuestaCorreo: (propuesta, correo) => {
        return new webix.promise((success, fail) => {
            devConfig.getConfig()
                .then(conf => {
                    var url = conf.urlApi + "/api/propuestas/correo";
                    return webix.ajax()
                        .timeout(10000)
                        .headers({
                            "Content-Type": "application/json"
                        })
                        .put(url, {propuesta: propuesta, correo: correo});
                })
                .then(function (result) {
                    success(result.json());
                })
    ...
    deleteLineaPropuesta: (propuestaLineaId, propuestaId) => {
        return new webix.promise((success, fail) => {
            devConfig.getConfig()
                .then(conf => {
                    var url = conf.urlApi + "/api/propuestas/lineas/" + propuestaLineaId + "/" + propuestaId;
                    return webix.ajax()
                        .timeout(10000)
                        .headers({
                            "Content-Type": "application/json"
