...
export const usuarioService = {
    ...
    checkLoggedUserExterno: () => {
        // Auth url
        // Verify if exists a user cookie
        var usu = usuarioService.getUsuarioCookieExterno();
        var authUrl = "/#!/loginExterno";
        if (!usu) {
            window.open(authUrl, '_blank');
            window.close();
            return false;
      ...
      getUsuarioCookieExterno: () => {
        var usuario = cookieApi.getCookie('usuarioArquitecturaExterno');
        if (!usuario) return null;
        return JSON.parse(usuario);
    },
    // setUsuarioCookie
    // Saves usuario's information in a cookie
    setUsuarioCookieExterno: (usuario) => {
        cookieApi.setCookie('usuarioArquitecturaExterno', JSON.stringify(usuario), 1);
    },
    deleteUsuarioCookieExterno: (usuario) => {
        cookieApi.deleteCookie('usuarioArquitecturaExterno');
    },


    getAccesoCookieExterno: () => {
        var Acceso = cookieApi.getCookie('loginArquitecturaExterno');
        if (!Acceso) return null;
        return JSON.parse(Acceso);
    },

    setAccesoCookieExterno: (Acceso) => {
        cookieApi.setCookie('loginArquitecturaExterno', JSON.stringify(Acceso), 100000);
    },
    deleteAccesoCookieExterno: () => {
        cookieApi.deleteCookie('loginArquitecturaExterno');
    },
